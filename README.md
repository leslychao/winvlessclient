...
- Built-in proxy domain coverage comes from the tracked `settings.json` seed; if that file is unavailable, the app falls back to the minimal built-in set: `youtube.com`, `youtu.be`, `googlevideo.com`, `ytimg.com`, `openai.com`, `chatgpt.com`, `oaistatic.com`.
- Domain entries are canonicalized and consolidated before routing: `*.example.com` is stored as `example.com`, and `api.example.com` is dropped when `example.com` is already present because `domain_suffix` already matches subdomains.
...
- The project-root `settings.json` is a tracked seed/default domain list. User-edited runtime domain preferences are saved to `runtime/settings.json` (ignored by git) and are merged with updated seed domains on load/save.
